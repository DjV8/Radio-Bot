...
client.on("ready", () => { // Inicjacja bota
    ...
    setInterval( async () => {
        ...
        client.user.setPresence({  // prezencja https://discord.js.org/#/docs/main/stable/typedef/PresenceData
            activity: {
...
const logger = winston.createLogger({ // logger winston
    level: "info",
...
const rl = readline.createInterface({ // interfejs konsoli
    input: process.stdin,
    output: process.stdout
})

client.on("message", async message => { // główny handler wiadomości
    if (message.author.bot) return;

    if (message.content.toLowerCase().includes("twoja stara")) {
        if (!message.channel.permissionsFor(message.client.user).has("SEND_MESSAGES")) {
            message.author.send("Mordo nie mogę pisać")
    ...
    if (!message.content.startsWith("<@" + client.user + ">") && !message.content.startsWith("<@!" + client.user + ">")) {
        return;
    }
    ...
    if (args[1] === "odśwież" && (message.author.id === "409704685969342503" || message.author.id === "304668018108137472")) {
...
async function execute(message, serverQueue) {
    ...
    if (!permissions.has("CONNECT") || !permissions.has("SPEAK")) return message.channel.send("No bym wbił ale nie moge 😕");
    if (serverQueue) {
    ...
    if (args[2].includes("youtube.com") || args[2].includes("youtu.be")) { //sprawdz czy youtube
        try {
...
function skip(message, serverQueue) {
    ...
    if (serverQueue.voiceChannel != voiceChannel) return message.channel.send("Przecież ciebie nawet tu nie ma");
...
function play(guild) {
    ...
    if (serverQueue.media[0].yt) {
        dispatcher = serverQueue.connection.play(ytdl(serverQueue.media[0].url, { filter: 'audioonly', highWaterMark: 1 << 25 }))
    }
    else {
